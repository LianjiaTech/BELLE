...
It includes prompts corresponding to the test data for each category, and the eval_set.json test cases are integrated through the corresponding prompts for each category. The evaluation scores are obtained by calling ChatGPT or GPT-4.Considering that the final results for the code and math categories are usually unique, both prompt for these two categories contain a hint for ChatGPT that "the final answer given by the model must be consistent with the final answer in the standard answer." For the other categories, except for rewrite, generation, and brainstorming, which do not have a standard answer, the prompt instructs ChatGPT to refer to the standard answer. For the rewrite, generation, and brainstorming categories, the prompt is designed to provide a set of evaluation criteria for ChatGPT to study and evaluate the model's response.
